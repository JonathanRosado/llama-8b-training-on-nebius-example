...
variable "persistent_volume_claim_jail_submounts" {
  description = "Pre-created Kubernetes PVCs to be mounted inside jail on worker nodes. Use this for externally managed storage such as Mountpoint-S3 CSI volumes that should appear inside the Slurm jail without creating Nebius Filestore resources."
  type = list(object({
    name       = string
    mount_path = string
    claim_name = string
