...
variable "persistent_volume_claim_jail_submounts" {
  description = "Pre-created Kubernetes PVCs to be mounted inside the Slurm jail on worker nodes. This is the escape hatch for non-Filestore shared storage such as a Mountpoint-S3 CSI PersistentVolume."
  type = list(object({
    name       = string
    mount_path = string
    claim_name = string
